                        ...
                        @if($this->searchableColumns()->count())
                            <div class="flex rounded-lg w-96 shadow-sm">
                                <div class="relative flex-grow focus-within:z-10">
                                    ...
                                    <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Search Here')}}" type="text" />
            ...
            <div class="max-w-screen bg-white @unless($complex) rounded-b-lg @endunless border-4 border-t-0 border-b-0 @if($this->activeFilters) border-blue-500 @else border-transparent @endif">
                <div class="items-center justify-between p-2 sm:flex">
                    ...
                    @if(count($this->results))
                        <div class="flex justify-end text-gray-600">
                            {{__('Showing')}} {{ $this->results->firstItem() }} to {{ $this->results->lastItem() }} {{__('of')}}
                            {{ $this->results->total() }} entries
                        </div>
