        ...
        <div class="table-responsive mt-3 my-team-details table-record">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-gray-500 text-xs">
                        {{ __('cruds.user.fields.referral_code')}}
                        <span wire:click="sortBy('referral_code')" class="float-right text-sm" style="cursor: pointer;">
                            <i class="fa fa-arrow-up {{ $sortColumnName === 'referral_code' && $sortDirection === 'asc' ? '' : 'text-muted' }}"></i>
                            <i class="fa fa-arrow-down m-0 {{ $sortColumnName === 'referral_code' && $sortDirection === 'desc' ? '' : 'text-muted' }}"></i>
                        </span>
                    </th>

                    <th class="text-gray-500 text-xs">
                        {{ __('cruds.user.fields.bonus_credits')}}
                        <span wire:click="sortBy('bonus_credits')" class="float-right text-sm" style="cursor: pointer;">
                            <i class="fa fa-arrow-up {{ $sortColumnName === 'bonus_credits' && $sortDirection === 'asc' ? '' : 'text-muted' }}"></i>
                            <i class="fa fa-arrow-down m-0 {{ $sortColumnName === 'bonus_credits' && $sortDirection === 'desc' ? '' : 'text-muted' }}"></i>
                        </span>
                    </th>
                ...
                @if($users->count() > 0)
                    ...
                    <tr>
                        ...
                        <td> 
                            @if($user->buyerPlan)
                            <div class="w-50">
                                <img src="{{$user->buyerPlan->image_url}}" class="img-fluid" alt="{{ $user->buyerPlan->title }}">
                            </div>
                            @endif  
                        </td>
                        ...
                        <td>
                            ...
                            <button type="button" title="{{ $user->referral_code ? 'Edit' : 'Add' }} REP code" data-id="{{$user->id}}" class="btn btn-primary btn-rounded btn-icon editRepCodeBtn">
                                <i class="ti-credit-card"></i>
                            </button>
                        </td>
