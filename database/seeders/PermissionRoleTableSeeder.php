...
class PermissionRoleTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $adminPermissions = $all_permissions->filter(function ($permission) {
            ...
            substr($permission->title, 0, 8) != 'plan_' && substr($permission->title, 0, 8) != 'setting_' && substr($permission->title, 0, 12) != 'transaction_' && substr($permission->title, 0, 11) != 'buyer_plan_' && substr($permission->title, 0, 18) != 'buyer_transaction_';
        });
