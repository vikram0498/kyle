...
class BuyersImport implements ToModel, WithStartRow
{
    ...
    public function model(array $row)
    {
        if($row[0] != 'First Name'){
            // dd($row);
            $buyerArr = [];
            $fName = $this->test_input($row[0]); $lName = $this->test_input($row[1]);
                ...
                if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL) ) {
                    ...
                    if(!$buyerExists){
                        ...
                        if(!empty($phone) && is_numeric($phone)){
                            ...
                            $address = $this->test_input($row[4]); $city = $this->test_input($row[5]); $state = $this->test_input($row[6]); $zipCode = $this->test_input($row[7]);
                            if(!empty($address) && !empty($city) && !empty($state) && !empty($zipCode)){
                                ...
                                $bedroomMin     = strtolower($this->test_input($row[11]));      $bedroomMax      = strtolower($this->test_input($row[12]));
                                $bathMin        = strtolower($this->test_input($row[13]));      $bathMax         = strtolower($this->test_input($row[14]));
                                $sizeMin        = strtolower($this->test_input($row[15]));      $sizeMax         = strtolower($this->test_input($row[16]));
                                $lotSizeMin     = strtolower($this->test_input($row[17]));      $lotSizeMax      = strtolower($this->test_input($row[18]));
                                $buildYearMin   = strtolower($this->test_input($row[19]));      $buildYearMax    = strtolower($this->test_input($row[20]));
                                $arvMin         = strtolower($this->test_input($row[21]));      $arvMax          = strtolower($this->test_input($row[22]));
                                ...
                                if(!empty($bedroomMin) && !empty($bedroomMax) && !empty($sizeMin) && !empty($sizeMax) ){
                                    if(is_numeric($bedroomMin) && is_numeric($bedroomMax) && is_numeric($sizeMin) && is_numeric($sizeMax)){
                                        ...
                                        $propertyType = strtolower($this->test_input($row[24])); 
                                        if(!empty($propertyType) && $propertyType != 'blank'){
                                            ...
                                            if(!empty($ptArr)){
                                                ...
                                                $buyerType = strtolower($this->test_input($row[40])); 
    ...
    private function setMultiSelectValues($value, $type, $buyerArr = []){
        if($type == 'property_type'){
            ...
            $parking = strtolower($this->test_input($value[23])); 
            ...
            $propertyFlaw = strtolower($this->test_input($value[25]));
    ...
    private function setCreativeBuyer($row, $buyerArr, $buyerTypeArr){
        $maxDownPaymentPercentage = strtolower($this->test_input($row[41]));
        $maxDownPaymentMoney = strtolower($this->test_input($row[42]));
        $maxInterestRate = strtolower($this->test_input($row[43])); 
        $balloonPayment = strtolower($this->test_input($row[44]));
    ...
    private function setMultiFamilyBuyer($row, $buyerArr){
        $unitMin = strtolower($this->test_input($row[45]));
        $unitMax = strtolower($this->test_input($row[46]));
        $buildingClass = strtolower($this->test_input($row[47])); 
        $valueAdd = strtolower($this->test_input($row[48]));
    ...
    private function setPurchaseMethod($row, $buyerArr){
        $purchaseMethod = strtolower($this->test_input($row[49]));
