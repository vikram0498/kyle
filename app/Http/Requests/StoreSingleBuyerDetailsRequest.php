...
class StoreSingleBuyerDetailsRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if(!empty($this->property_type) && !array_intersect([14], $this->property_type)){
            ...
            // $rules['arv_min'] = ['required', 'numeric', !empty($this->arv_max) ? new CheckMinValue($this->arv_max, 'arv_max') : ''];
            // $rules['arv_max'] = ['required', 'numeric', !empty($this->arv_min) ? new CheckMaxValue($this->arv_min, 'arv_min') : ''];
